...
@api_experimental.route('/dags/<string:dag_id>/enhanced_trigger', methods=['POST'])
@requires_authentication
def enhanced_trigger(dag_id):
    """
    Trigger a new dag run for a Dag with an execution date of now unless
    specified in the data.
    """
    try:
        dagruns, resp = tapd_enhanced_trigger_dag(dag_id, url_params=request.get_json(force=True))
    ...
    return jsonify({"dagruns":dagruns,"response": resp})
